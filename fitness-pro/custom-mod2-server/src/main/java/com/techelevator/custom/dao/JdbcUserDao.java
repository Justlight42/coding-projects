...
public class JdbcUserDao implements UserDao {
    ...
    public int deleteUser(int userId) {
        ...
        String sqlUserExercise = "DELETE FROM user_exercise WHERE user_id = ?;";
        String sqlUsers = "DELETE FROM users WHERE user_id = ?;";
        try {
            jdbcTemplate.update(sqlUserExercise, userId);
            jdbcTemplate.update(sqlReview, userId);
            return jdbcTemplate.update(sqlUsers, userId);
        } catch (CannotGetJdbcConnectionException e) {
            throw new DaoException("Error connecting to the server " + e);
        } catch (DataAccessException e) {
            throw new DaoException("Error deleting User using user ID: " + userId, e);
        }
